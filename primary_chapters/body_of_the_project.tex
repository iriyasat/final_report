...
\begin{itemize}
    \item \textbf{Operational Effects:} Automating sale recording and invoice generation substantially reduces transaction processing time, while direct customer messaging eliminates missed leads.
...
\phantomsection\label{lst:store_sales_sql}
\footnotesize
\begin{verbatim}
SELECT 
    s.store_id,
...
Automated unit and integration test suites executed via Django's test runner achieved complete pass execution across all 21 test routines, verifying model validation rules, custom numeric ID authentication, atomic holding reservations, permission boundaries, and API schemas.
