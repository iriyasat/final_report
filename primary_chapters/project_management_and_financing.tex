...
Overall, the schedule is front-loaded on database modeling and architecture to prevent costly downstream refactoring, while back-loading validation with a dedicated testing buffer prior to deployment.
...
As the summary illustrates, the largest share of effort, roughly 45\%, is allocated to Backend/API Development, reflecting the reality that Phases 3.0 through 8.0 (ERP, E-Commerce, Authentication, API, Analytics, and Messaging) are primarily server-side engineering tasks. QA/Testing effort (20\%) was distributed across nearly every phase rather than concentrated solely in Phase 10.0, reflecting an incremental validation practice alongside each functional module. Database Design (16.5\%) and Frontend Development (11.5\%) were front-loaded and back-loaded respectively, occurring predominantly in Phase 2.0 and Phase 9.0, while DevOps effort (7.4\%) was concentrated during initial environment setup and final production deployment.
